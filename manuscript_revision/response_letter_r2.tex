...
\begin{quote}
    % 第一段
    ``... Despite these advantages, the applicability of EDM-based methods rests on 
    specific assumptions regarding the underlying dynamical system. Specifically, 
    the method requires sufficient deterministic structure and may be sensitive to 
    short or irregular time series, measurement noise, and partial observability. 
    Crucially, inferred causal relationships are conditional on the selected set 
    of observed variables; omitting key confounders may lead to spurious conclusions.
    
    % 第二段
    Furthermore, we emphasize that the null hypothesis of ``no causal relationship'' 
    is often unrealistic in complex urban systems \citep{meehl1990why}. Cross-mapping 
    skill should therefore be interpreted as a measure of directional dynamical 
    dependence rather than a definitive causal effect. Robust inference requires 
    joint consideration of convergence, effect size, consistency across methods, 
    and domain knowledge.
    
    % 第三段
    Finally, the methodological extensions introduced here (multispatial CCM, 
    generalized PCM) remain subject to the same fundamental assumptions of the 
    EDM framework. Users must ensure comparability and dynamical consistency across 
    spatial units and verify stable reconstruction quality before drawing conclusions.''
\end{quote}
\endgroup
